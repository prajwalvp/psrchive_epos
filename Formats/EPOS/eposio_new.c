...
int get_data(const char* filename, eposdata_t *eposdata, unsigned isub, int verbose)
{
  ...
  nbytes = fread(ch2, 1, sizeof(ch2), fp);
  nbytes = fread(ch3, 1, sizeof(ch3), fp);
  ...
  for ( i=0; i< 1024; i++){
	...
	eposdata->data1[i] = ((ch1[i] & 0x000000ff) << 16) | ((ch1[i] & 0x0000ff00) << 16) |\
	  ((ch1[i] & 0x00ff0000) >> 16) | ((ch1[i] & 0xff000000) >> 16) ;
        ...
        eposdata->data2[i] = ((ch2[i] & 0x000000ff) << 16) | ((ch2[i] & 0x0000ff00) << 16) |\
          ((ch2[i] & 0x00ff0000) >> 16) | ((ch2[i] & 0xff000000) >> 16) ;

        eposdata->data3[i] = ((ch3[i] & 0x000000ff) << 16) | ((ch3[i] & 0x0000ff00) << 16) |\
          ((ch3[i] & 0x00ff0000) >> 16) | ((ch3[i] & 0xff000000) >> 16) ;
